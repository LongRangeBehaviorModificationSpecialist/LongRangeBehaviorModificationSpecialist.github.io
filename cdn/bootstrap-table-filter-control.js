...
!(function ($) {
      ...
      if (header.find(".date-filter-control").length > 0) {
        $.each(that.columns, function (i, column) {
          if (
            ...
            header
              .find(".date-filter-control." + column.field)
    ...
    for (var i = this.pageFrom - 1; i < pageTo; i++) {
      ...
      $.each(this.header.fields, function (j, field) {
        var value = item[field],
          column =
            that.columns[
              $.fn.bootstrapTable.utils.getFieldIndex(that.columns, field)
            ];
          ...
          if (
            ...
            if (
              ...
              if (selectControl !== undefined && selectControl.length > 0) {
                if (
                  selectControl.get(selectControl.length - 1).options.length ===
                  0
                ) {
    ...
    this.data = fp
      ? $.grep(this.data, function (item, i) {
          for (var key in fp) {
            var thisColumn =
              that.columns[
                $.fn.bootstrapTable.utils.getFieldIndex(that.columns, key)
              ];
  ...
  BootstrapTable.prototype.clearFilterControl = function () {
    if (this.options.filterControl && this.options.filterShowClear) {
      ...
      var controls = getCurrentHeader(this).find(
        getCurrentSearchControls(this)
      );
      timeoutId = 0;
